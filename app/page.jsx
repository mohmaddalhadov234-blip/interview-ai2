        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(160px,1fr))", gap: 10, marginBottom: 24 }}>
          {JOBS.map(j => (
            <div key={j.id} onClick={() => setSelectedJob(j.id)}
              style={{ background: selectedJob === j.id ? "linear-gradient(135deg," + j.color + "22," + j.color + "11)" : cardBg, border: "1px solid " + (selectedJob === j.id ? j.color + "66" : cardBorder), borderRadius: 14, padding: "14px 12px", cursor: "pointer" }}>
    ...
    <div style={{ height: "100dvh", display: "flex", flexDirection: "column", background: bg, color: textColor, fontFamily: "sans-serif", overflow: "hidden" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 16px", borderBottom: "1px solid " + cardBorder, flexShrink: 0, flexWrap: "wrap", gap: 8 }}>
        ...
        <button onClick={finishInterview} disabled={userAnswers < 2 || loading}
          style={{ background: userAnswers >= 2 && !loading ? "linear-gradient(135deg,#10B981,#059669)" : cardBg, border: "none", borderRadius: 8, padding: "6px 12px", cursor: userAnswers >= 2 && !loading ? "pointer" : "not-allowed", color: userAnswers >= 2 && !loading ? "#fff" : "rgba(128,128,128,0.4)", fontSize: 12, fontWeight: 700 }}>
          📊 Результат
        </button>
      </div>
      ...
      <div style={{ padding: "12px 14px", borderTop: "1px solid " + cardBorder, flexShrink: 0 }}>
        <div style={{ display: "flex", gap: 9, alignItems: "flex-end" }}>
          <textarea value={input} onChange={e => setInput(e.target.value)}
            ...
            style={{ flex: 1, background: cardBg, border: "1px solid " + cardBorder, borderRadius: 13, padding: "10px 13px", color: textColor, fontSize: 14, resize: "none", outline: "none", lineHeight: 1.5 }}
          />
          <button onClick={handleSend} disabled={!input.trim() || loading}
            style={{ width: 44, height: 44, flexShrink: 0, background: input.trim() && !loading ? "linear-gradient(135deg,#4F6EF7,#A855F7)" : cardBg, border: "1px solid " + cardBorder, borderRadius: 13, cursor: input.trim() && !loading ? "pointer" : "not-allowed", fontSize: 17, display: "flex", alignItems: "center", justifyContent: "center" }}>➤</button>
        </div>
